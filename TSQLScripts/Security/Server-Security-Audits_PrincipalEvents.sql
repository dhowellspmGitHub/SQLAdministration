...
SET @AuditName = N'Server-PrincipalsEvents'
SET @AuditSpecName = N'Server-PrincipalsEventSpecifications'
...
BEGIN
	...
	CREATE SERVER AUDIT [Server-PrincipalsEvents]
	TO FILE 
	...
	ALTER SERVER AUDIT [Server-PrincipalsEvents] WITH (STATE = OFF);

	/* Enable the audit */
	IF @AuditState = 0
	BEGIN
		PRINT N'Enabling Server Audit'	
		ALTER SERVER AUDIT [Server-PrincipalsEvents] WITH (STATE = ON);
	...
	ELSE
	BEGIN 
		CREATE SERVER AUDIT SPECIFICATION [Server-PrincipalsEventSpecifications]
		FOR SERVER AUDIT [Server-PrincipalsEvents]
	...
	BEGIN
		...
		ALTER SERVER AUDIT SPECIFICATION [Server-PrincipalsEventSpecifications] WITH (STATE = ON)
	END
	ELSE
	BEGIN
