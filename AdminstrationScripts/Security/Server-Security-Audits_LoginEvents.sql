...
SET @AuditName = N'Server-LogOnEvents'
SET @AuditSpecName = N'Server-LogOnEventSpecifications'
...
BEGIN
	...
	CREATE SERVER AUDIT [Server-LogOnEvents]
	TO FILE 
	...
	ALTER SERVER AUDIT [Server-LogOnEvents] WITH (STATE = OFF);
		/* Enable the audit */
	IF @AuditState = 0
	BEGIN
		PRINT N'Enabling Server Audit'	
		ALTER SERVER AUDIT [Server-LogOnEvents] WITH (STATE = ON);
	...
	ELSE
	BEGIN 
		CREATE SERVER AUDIT SPECIFICATION [Server-LogOnEventSpecifications]
		FOR SERVER AUDIT [Server-LogOnEvents]
	...
	IF @AuditState = 0
	BEGIN
		PRINT N'Enabling Server Audit Specification'	
		ALTER SERVER AUDIT SPECIFICATION [Server-LogOnEventSpecifications] WITH (STATE = ON)
	
	END
	BEGIN
		PRINT N'Server Audit Specification:' + @AuditSpecName + ' already enabled'
