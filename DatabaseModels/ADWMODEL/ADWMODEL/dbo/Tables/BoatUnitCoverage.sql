CREATE TABLE [dbo].[BoatUnitCoverage] (
    ...
    CONSTRAINT [PK_BoatUnitCoverage] PRIMARY KEY CLUSTERED ([PolicyId] ASC, [UnitNbr] ASC, [CoverageCd] ASC) ON [POLICYCD],
    CONSTRAINT [FK_BoatUnitCoverage_BoatUnit_01] FOREIGN KEY ([PolicyId], [UnitNbr]) REFERENCES [dbo].[BoatUnit] ([PolicyId], [UnitNbr])
) ON [POLICYCD];


GO
CREATE NONCLUSTERED INDEX [IX_BoatUnitCoverage_01]
    ON [dbo].[BoatUnitCoverage]([PolicyId] ASC)
    ON [POLICYCI];
