CREATE TABLE [dbo].[CommercialBuildingUnitEndorsement] (
    ...
    CONSTRAINT [PK_CommercialBuildingUnitEndorsement] PRIMARY KEY CLUSTERED ([PolicyId] ASC, [EndorsementNbr] ASC, [UnitNbr] ASC, [EndorsementId] ASC) ON [POLICYCD],
    CONSTRAINT [FK_CommercialBuildingUnitEndorsement_CommercialBuildingUnit_01] FOREIGN KEY ([PolicyId], [UnitNbr]) REFERENCES [dbo].[CommercialBuildingUnit] ([PolicyId], [UnitNbr]),
    CONSTRAINT [FK_CommercialBuildingUnitEndorsement_Endorsements_01] FOREIGN KEY ([EndorsementId]) REFERENCES [dbo].[Endorsements] ([EndorsementId])
) ON [POLICYCD];


GO
CREATE NONCLUSTERED INDEX [IX_CommercialBuildingUnitEndorsement_01]
    ON [dbo].[CommercialBuildingUnitEndorsement]([PolicyId] ASC)
    ON [POLICYCI];
