CREATE TABLE [dbo].[CommercialPolicyCoverage] (
    ...
    CONSTRAINT [PK_CommercialPolicyCoverage] PRIMARY KEY CLUSTERED ([PolicyId] ASC, [CoverageCd] ASC) ON [POLICYCD],
    CONSTRAINT [FK_CommercialPolicyCoverage_CommercialPolicy_01] FOREIGN KEY ([PolicyId]) REFERENCES [dbo].[CommercialPolicy] ([PolicyId])
) ON [POLICYCD];


GO
CREATE NONCLUSTERED INDEX [IX_CommercialPolicyCoverage_01]
    ON [dbo].[CommercialPolicyCoverage]([PolicyId] ASC)
    ON [POLICYCI];
