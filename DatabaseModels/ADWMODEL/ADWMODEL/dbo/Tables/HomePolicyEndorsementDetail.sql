CREATE TABLE [dbo].[HomePolicyEndorsementDetail] (
    ...
    CONSTRAINT [PK_HomePolicyEndorsementDetail] PRIMARY KEY CLUSTERED ([EndorsementDetailId] ASC) ON [POLICYCD],
    CONSTRAINT [FK_HomePolicyEndorsementDetail_AdditionalInterest_01] FOREIGN KEY ([AdditionalInterestId]) REFERENCES [dbo].[AdditionalInterest] ([AdditionalInterestId]),
    CONSTRAINT [FK_HomePolicyEndorsementDetail_HomePolicyEndorsement_01] FOREIGN KEY ([PolicyId], [EndorsementId]) REFERENCES [dbo].[HomePolicyEndorsement] ([PolicyId], [EndorsementId])
) ON [POLICYCD] TEXTIMAGE_ON [POLICYCD];


GO
CREATE NONCLUSTERED INDEX [IX_HomePolicyEndorsementDetail_01]
    ON [dbo].[HomePolicyEndorsementDetail]([PolicyId] ASC)
    ON [POLICYCI];
