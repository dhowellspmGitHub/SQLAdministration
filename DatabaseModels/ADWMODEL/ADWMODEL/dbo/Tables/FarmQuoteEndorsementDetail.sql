CREATE TABLE [dbo].[FarmQuoteEndorsementDetail] (
    ...
    CONSTRAINT [PK_FarmQuoteEndorsementDetail] PRIMARY KEY CLUSTERED ([EndorsementDetailId] ASC, [QuoteNbr] ASC, [EndorsementId] ASC) ON [POLICYCD],
    CONSTRAINT [FK_FarmQuoteEndorsementDetail_FarmQuoteEndorsement_01] FOREIGN KEY ([QuoteNbr], [EndorsementId]) REFERENCES [dbo].[FarmQuoteEndorsement] ([QuoteNbr], [EndorsementId]),
    CONSTRAINT [FK_FarmQuoteEndorsementDetail_Location_01] FOREIGN KEY ([LocationID]) REFERENCES [dbo].[Location] ([LocationID])
) ON [POLICYCD] TEXTIMAGE_ON [POLICYCD];


GO
CREATE NONCLUSTERED INDEX [IX_FarmQuoteEndorsementDetail_01]
    ON [dbo].[FarmQuoteEndorsementDetail]([QuoteNbr] ASC)
    ON [POLICYCI];
