USE [tempdb]
GO
/* Create tables in the tempdb database to record DBPs, DBRs, DBRMs and DBR authorization before any backup/restore process is tried.
These tables are used in validating the backups and restore process all records.  In the validation, all are compared to the DB access and authorization tables 
*/
